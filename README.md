...
3. Приложение будет доступно на `http://localhost:7878`, PostgreSQL — на `localhost:5432`, Redis — на `localhost:6379`, воркер очереди запустится как отдельный сервис `worker`.
...
### Nginx-прокси
Сервис `nginx` проксирует домен из `NGINX_SERVER_NAME` (по умолчанию `example.com`) на API. Порт хоста — `80`. Для работы с вашим доменом пропишите реальное имя в `.env` (`NGINX_SERVER_NAME=your-domain.tld`) и укажите нужный апстрим, если меняете схему/порт (`API_UPSTREAM`, по умолчанию `http://api:7878`). После `docker compose up -d` Nginx доступен на 80 порту и раздаёт API и статические файлы.
